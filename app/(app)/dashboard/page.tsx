...
import { getUtmifySummary } from "@/lib/utmify";
...
export default async function Dashboard() {
  ...
  const utmify = await getUtmifySummary();
  let sales: Record<string, unknown>[] = [];
  let spend: Record<string, unknown>[] = [];
  if (supabase && !utmify) [sales, spend] = await Promise.all([
    supabase.from("sales").select("net_revenue,gross_revenue,sold_at").order("sold_at", { ascending: true }).limit(180).then(result => result.data || []),
    supabase.from("ad_spend").select("amount,spent_at").order("spent_at", { ascending: true }).limit(180).then(result => result.data || []),
  ]);
  const gross = utmify?.gross ?? sales.reduce((sum, item) => sum + Number(item.gross_revenue || 0), 0);
  const net = utmify?.net ?? sales.reduce((sum, item) => sum + Number(item.net_revenue || 0), 0);
  const costs = utmify?.costs ?? spend.reduce((sum, item) => sum + Number(item.amount || 0), 0);
  const formatter = new Intl.NumberFormat("pt-BR", { style: "currency", currency: utmify?.currency || "BRL" });
  const metrics = [
    { label: "Faturamento bruto", value: formatter.format(gross), icon: CircleDollarSign, tone: "text-[var(--gold)]" },
    { label: "Faturamento líquido", value: formatter.format(net), icon: ReceiptText, tone: "text-[#e4d7b4]" },
    { label: "Gastos", value: formatter.format(costs), icon: MousePointerClick, tone: "text-[#d5b6b1]" },
    { label: "Lucro", value: formatter.format(utmify?.profit ?? net - costs), icon: TrendingUp, tone: "text-[#9bd2ac]" },
    { label: "Vendas", value: String(utmify?.sales ?? sales.length), icon: ShoppingBag, tone: "text-[#c4c9e8]" },
    { label: "ROAS", value: utmify?.roas ? `${utmify.roas.toFixed(2)}x` : costs ? `${(net / costs).toFixed(2)}x` : "—", icon: TrendingUp, tone: "text-[var(--gold)]" },
  ];
  ...
  const chart = utmify?.chart || Array.from(byDate.values()).map(item => ({ ...item, lucro: item.receita - item.gastos }));
